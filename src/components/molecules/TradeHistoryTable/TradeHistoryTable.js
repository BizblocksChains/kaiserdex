...
import AndroidSync from 'react-icons/lib/io/android-sync';
...
const TradeHistoryTable = ({data, personal, onCancelOrder, onScroll, hasNext, forPage, showTooltip, onSync}) => {
    ...
    <div className={cx('trade-history-table-wrapper')}>
      <div className={cx('trade-history-table', { forPage })}>
        { !forPage && <div className={cx('title')}>
          {personal ? <button className={cx('button')} onClick={onSync}>
          <AndroidSync onClick={() => onSync} /></button> : '' } 
          {personal ? ' 나의 ' : '' }거래내역 
        </div> }
