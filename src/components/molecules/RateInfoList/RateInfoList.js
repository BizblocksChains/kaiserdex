...
import styles from './RateInfoList.scss';
...
class RateInfoList extends Component {
    ...
    if(currencyKey === 'BTC') return null;
      ...
      return (
        <div className={cx('wrapper')}>
          <HoverCard className={cx('rate-info-list', highlight && (greater ? 'green' : 'red'))} 
          onClick={handleOpenCurrency}>
            ...
            <div className={cx('left-items')}>
              <span className={cx('pin-wrapper', { active: pinned })}><PinIcon onClick={(e) => { e.stopPropagation(); onTogglePin(); }}/></span>
              <span className={cx('short-name')}> {currencyKey} </span>
              <span className={cx('percentage', { positive: parsedPercentage > 0, netural: parsedPercentage === 0 })}>({parsedPercentage.toFixed(2)}%)</span>
              {/* <span className={cx('volume')}>  (volume) </span> */}
            </div>
            <div className={cx('right-items')}>
              <span className={cx('usd')}> {value} </span>
              <span className={cx('name')}> {currencyName} {parsedVolume} </span>
            </div>
          </HoverCard>
        </div>
      );
...
export default withRouter(RateInfoList);