...
class TradeHistoryContainer extends Component {
  ...
  handleCancelOrder = async (id) => {
    ...
    try {
      ...
      await TradeActions.getOrders({currencyPair: currencyType === 'BTC' ? 'USDT_BTC' : `BTC_${currencyType}`});
      await UserActions.getWallet();
    }
  }

  handleSync = async () => {
    const { TradeActions, UserActions, currencyType } = this.props;
    try {
      await TradeActions.getOrders({currencyPair: currencyType === 'BTC' ? 'USDT_BTC' : `BTC_${currencyType}`});
      await UserActions.getWallet();
    } catch (e) {
  ...
  render() {
    const { handleCancelOrder, handleScroll, handleSync } = this;
